...
SAMPLE_COURSES: list[dict] = [
    ...
    {"course_name": "AWS Cloud Practitioner Essentials", "organization": "Amazon Web Services", "description": "Build foundational knowledge of AWS cloud concepts, AWS services, security, architecture, pricing, and support.", "skills": ["AWS", "Cloud Computing", "Cloud Architecture", "Security", "Networking"], "difficulty_level": "Beginner", "rating": 4.7},
    ...
    {"course_name": "IBM Data Engineering Professional Certificate", "organization": "IBM", "description": "Build job-ready skills for a data engineering career. Learn Python, SQL, NoSQL, big data, Spark, ETL, and data warehousing on the cloud.", "skills": ["Python", "SQL", "Apache Spark", "NoSQL", "ETL", "Data Engineering"], "difficulty_level": "Intermediate", "rating": 4.6},
    ...
    {"course_name": "Meta Front-End Developer Certificate", "organization": "Meta", "description": "Launch a career as a front-end developer. Build dynamic web pages using HTML, CSS, JavaScript, and React.", "skills": ["HTML", "CSS", "JavaScript", "React", "Node.js", "REST APIs"], "difficulty_level": "Beginner", "rating": 4.7},
    {"course_name": "Full Stack Web Development with React", "organization": "HKUST", "description": "Learn Bootstrap, React, Angular, Node.js, Express.js, and MongoDB to build complete full-stack web applications.", "skills": ["React", "Node.js", "JavaScript", "MongoDB", "Express.js", "REST APIs"], "difficulty_level": "Intermediate", "rating": 4.5},
    ...
    {"course_name": "SQL for Data Science", "organization": "UC Davis", "description": "Learn how to read and write intermediate-level SQL queries using the SELECT statement for data science applications.", "skills": ["SQL", "Data Analysis", "Database Management", "SQLite", "Data Filtering"], "difficulty_level": "Beginner", "rating": 4.6},
    ...
    {"course_name": "MLOps Specialization", "organization": "DeepLearning.AI", "description": "Learn to build, deploy, and maintain production ML systems including feature engineering, model training pipelines, and monitoring.", "skills": ["MLOps", "Python", "TensorFlow", "Docker", "Kubernetes", "ML Pipelines"], "difficulty_level": "Advanced", "rating": 4.7},
    {"course_name": "Docker and Kubernetes: The Complete Guide", "organization": "Udemy", "description": "Build production-grade web applications and deploy them using Docker and Kubernetes. Learn container orchestration from scratch.", "skills": ["Docker", "Kubernetes", "DevOps", "CI/CD", "Node.js", "AWS"], "difficulty_level": "Intermediate", "rating": 4.8},
    ...
    {"course_name": "Computer Vision with TensorFlow", "organization": "DeepLearning.AI", "description": "Build computer vision applications using CNNs, object detection, segmentation, and transfer learning with TensorFlow.", "skills": ["Computer Vision", "TensorFlow", "CNNs", "Object Detection", "Python", "Deep Learning"], "difficulty_level": "Intermediate", "rating": 4.6},
    {"course_name": "Reinforcement Learning Specialization", "organization": "University of Alberta", "description": "Master RL algorithms including Q-learning, policy gradients, actor-critic, and apply them to complex decision-making problems.", "skills": ["Reinforcement Learning", "Python", "Machine Learning", "Neural Networks"], "difficulty_level": "Advanced", "rating": 4.6},
    {"course_name": "Business Analytics Specialization", "organization": "University of Pennsylvania", "description": "Learn how to apply data analytics to real business problems including customer analytics, people analytics, and accounting analytics.", "skills": ["Business Intelligence", "Data Analysis", "Excel", "Tableau", "Statistics", "SQL"], "difficulty_level": "Beginner", "rating": 4.6},
    {"course_name": "Apache Spark and Scala", "organization": "Databricks", "description": "Master Apache Spark using Scala for large-scale distributed data processing, streaming, machine learning pipelines, and graph processing.", "skills": ["Apache Spark", "Scala", "Big Data", "Data Engineering", "MLlib"], "difficulty_level": "Advanced", "rating": 4.7},
...
# Column name aliases for coursea_data.csv → internal names
_COLUMN_ALIASES: dict[str, str] = {
    ...
    # pass-through aliases (already correct naming)
...
def _normalize_row(raw: dict) -> dict:
    """Map external column names (e.g. coursea_data.csv) to internal field names."""
    normalized: dict = {}
    for k, v in raw.items():
        key_lower = k.strip().lower().replace(" ", "_")
        mapped = _COLUMN_ALIASES.get(key_lower, key_lower)
        normalized[mapped] = v
    return normalized
...
def _build_course(row: dict) -> Course:
    ...
    title = clean_text(str(row.get("course_name", "Untitled Course")))
    organization = clean_text(str(row.get("organization", "Unknown")))

    # Resolve skills: explicit column → derive from title
    skills_raw = row.get("skills", "")
    if isinstance(skills_raw, list):
        skills = [s.strip() for s in skills_raw if str(s).strip()]
    elif skills_raw and str(skills_raw).strip() not in ("", "nan"):
        skills = parse_skills_list(str(skills_raw))
    else:
        skills = []

    if not skills:
        description_text = str(row.get("description", ""))
        if description_text and description_text not in ("", "nan"):
            skills = extract_skills_from_text(description_text)
        if not skills:
            skills = derive_skills_from_title(title, organization)

    # Generate description if missing (common with coursea_data.csv)
    description = clean_text(str(row.get("description", "")))
    if not description or description in ("nan", ""):
        cert = str(row.get("certificate_type", "")).replace("nan", "").strip()
        diff = parse_difficulty(str(row.get("difficulty_level", "Mixed")))
        skills_preview = ", ".join(skills[:4])
        description = (
            f"{title} offered by {organization}."
            + (f" {cert} program." if cert else "")
            + f" {diff}-level course covering {skills_preview}."
            if skills_preview else ""
        )

    certificate_type = str(row.get("certificate_type", "")).replace("nan", "").strip()
    students_enrolled = parse_enrolled(row.get("students_enrolled", 0))
    ...
    return Course(
        ...
        course_url=str(row.get("course_url", "")).replace("nan", ""),
...
async def ingest_courses(file_path: str = "data/courses.csv") -> int:
    path = Path(file_path)
    if not path.exists():
        # Try the Coursera dataset as fallback
        coursea_path = Path("data/coursea_data.csv")
        if coursea_path.exists():
            logger.info("Using coursea_data.csv dataset", path=str(coursea_path))
            return await ingest_courses(str(coursea_path))
    ...
    if path.suffix.lower() == ".csv":
        ...
        # Drop unnamed index columns (e.g. first column in coursea_data.csv)
